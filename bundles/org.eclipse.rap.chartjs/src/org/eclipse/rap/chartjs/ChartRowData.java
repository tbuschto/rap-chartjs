...
public class ChartRowData {
	...
	public ChartRowData addRow(float[] row, ChartStyle colors) {
		rows.add(row);
	...
	JsonObject toJson() {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			rowsJson.add(new JsonObject().add("fillColor", asCss(rowColors.getFillColor(), rowColors.getFillOpacity()))
					.add("strokeColor", asCss(rowColors.getStrokeColor()))
					.add("pointColor", asCss(rowColors.getPointColor()))
					.add("pointStrokeColor", asCss(rowColors.getPointStrokeColor())).add("data", asJson(rows.get(i))));
		}
