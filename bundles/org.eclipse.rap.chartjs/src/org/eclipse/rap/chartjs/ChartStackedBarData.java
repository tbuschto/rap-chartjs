...
public class ChartStackedBarData {

  private final String[] labels;
  private final List<int[]> rows = new ArrayList<int[]>( 1 );
  private final List<ChartStyle> colors = new ArrayList<ChartStyle>( 1 );

  public ChartStackedBarData( String[] labels ) {
    this.labels = labels;
  }

  public ChartStackedBarData addRow( int[] row, ChartStyle colors ) {
    rows.add( row );
  ...
  JsonObject toJson() {
    ...
    for( int i = 0; i < rows.size(); i++ ) {
      ...
      rowsJson.add( new JsonObject()
        ...
        .add( "highlightFill", asCss( rowColors.getPointColor() ) )
        .add( "highlightStroke", asCss( rowColors.getPointStrokeColor() ) )
